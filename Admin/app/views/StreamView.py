...
def player(request):
    ...
    # return HttpResponseJson(context)
...
# todo 这里的data是返回所有的在线视频流
def __getAllOnlineStream(is_filter_analyzer=False):
    data = []
    online_data = g_media.getMediaList()  # 获得所有在线的视频
...
def play_all_rtsp(request):
    context = {'all_stream': [],
               'index': []
               }
    mediaServerState, datas = __getAllOnlineStream(is_filter_analyzer=True)
    code = 1000
    msg = "success"
    top_msg = ""
    if not mediaServerState:
        top_msg = "流媒体服务未运行"
    context["is_exist_stream"] = 2
    for ind, k in enumerate(reversed(datas)):
        app = k["app"]
        name = k["name"]
        stream = GetStream(app, name)
        context["all_stream"].append(stream)
        context["index"].append(ind)
